...
print("UAS left branching: ", uas_metric.result())
uas_metric.reset_state()
uas_metric(range(1000), right_branching)
print("UAS right branching: ", uas_metric.result())
...
for idx, root in enumerate(dependency_trees.roots):
	...
	if root != last_tok_id :
		...
		gr_right_branching[idx].append((root , 0))
		
	if root != 0:
		gr_left_branching[idx].remove((root, root-1))
		gr_left_branching[idx].append((root, -1))
		gr_left_branching[idx].remove((0, -1))
		gr_left_branching[idx].append((root, last_tok_id))

uas_metric.reset_state()
uas_metric(range(1000), gr_left_branching)
print("UAS left branching (with gold root): ", uas_metric.result())
uas_metric.reset_state()
uas_metric(range(1000), gr_right_branching)
print("UAS right branching (with gold root): ", uas_metric.result())
